...
def fibertel() -> None:
    ...
    for prefix in home_prefixes:
        for i in range (1000000, 50000000):
            fibertel.write('{0:03d}{1:08d}\n'.format(int(prefix), int(i)))
            fibertel.write('{0:03d}{1:07d}\n'.format(int(prefix), int(i))) # Certain confution exists about if the DNI is the whole number or the first 7 digits, so I will double the posibilities.
    fibertel.close()
    print("Done")
...
def fibercorp() -> None:
    """
        Generates Fibertel Corporative Networks wordlist
    """
